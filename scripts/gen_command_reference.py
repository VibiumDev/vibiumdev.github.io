...
def emit_index(nightly: Binary, stable: Binary, stable_tops: set[str]) -> str:
    # The sidebar label differs from the title so the autogenerated
    # "Command Reference" group does not list a child with its own name.
    parts = [
    ...
    for top in nightly.top_level:
        ...
        # The marker lives in the description cell; in the command cell it
        # wraps onto its own line under the link.
        badge = " **(nightly)**" if top not in stable_tops else ""
        rows.append(f"| [`vibium {top}`]({top}.md) | {desc}{badge} |")
    parts.append("\n".join(rows))
    parts.append(
